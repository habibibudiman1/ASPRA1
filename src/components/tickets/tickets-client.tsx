...
export function TicketsClient({ tickets, pagination, categories, profile, currentFilters }: TicketsClientProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={currentFilters.status} onValueChange={(v) => updateFilter('status', v)}>
          <SelectTrigger className="w-full sm:w-40" id="filter-status">
        ...
        <Select value={currentFilters.priority} onValueChange={(v) => updateFilter('priority', v)}>
          <SelectTrigger className="w-full sm:w-40" id="filter-priority">
        ...
        {hasActiveFilters && (
          <Button variant="outline" size="icon" onClick={() => router.push(pathname)} id="clear-filters">
            <X className="h-4 w-4" />
          </Button>
        )}
      </div>
        ...
        <div className="space-y-3">
          {tickets.map((ticket) => (
            <Link href={`/tickets/${ticket.id}`} key={ticket.id} id={`ticket-${ticket.id}`}>
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 border rounded-xl bg-card hover:border-primary/30 hover:shadow-sm transition-all duration-150 cursor-pointer group">
                {/* Info Utama */}
                <div className="flex-1 min-w-0 space-y-1.5">
                  ...
                  <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
                    <span>Oleh: {ticket.reporter?.full_name ?? '-'}</span>
                    {ticket.assignee && <span>• Assignee: {ticket.assignee.full_name}</span>}
